...
export const fetchInactiveDepartments = async () => {
        ...
        if (personData) {
          ...
          if (personData.AccountType === 'Employee') {
            ...
            if (deptSnapshot.exists()) {
              ...
              let inactiveDepartments = [];

              for (const key in deptData) {
                const employee = deptData[key];
                if (employee.EmployeeId === personData.PersonId && employee.Active === false) {
                  inactiveDepartments.push(employee);
                }
              }

              // Eğer aktif olmayan departmanlar varsa, bunların bilgilerini döndür
